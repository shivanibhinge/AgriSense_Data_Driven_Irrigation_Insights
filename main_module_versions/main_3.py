...
with tabs[0]:
    ...
    st.markdown("**GitHub:** https://github.com/shivanibhinge/AgriSense_Data_Driven_Irrigation_Insights/tree/main")
...
with tabs[3]:
    ...
    st.markdown("""
    Principal Component Analysis (PCA) is an unsupervised technique that reduces the number of features
    in a dataset while keeping as much of the original information as possible. It works by finding new
    axes : called principal components : that capture the directions of maximum variance in the data.
    The first component captures the most variance, the second captures the next most, and so on.
    By projecting the data onto just the top two or three components, the dataset can be visualized
    in 2D or 3D even when it originally had dozens of features. PCA is especially useful before
    clustering or visualization, since it reduces noise and removes redundancy from correlated features.
    """)
    st.divider()

    st.subheader("1. Dataset Used")
    st.markdown("""
    PCA was applied to the full merged dataset (2031 rows × 31 numeric features) after removing date and label columns.
    - All numeric columns selected : no label or date columns included
    - Columns with near-zero variance removed
    - All features normalized using StandardScaler (mean = 0, std = 1) before PCA
    """)
...
with tabs[4]:
    ...
    with col1:
        ...
        **KMeans**
        ...
        - Fast, scalable, works well on spherical clusters
        """)
    with col2:
        st.markdown("""
        **Hierarchical (Ward)**
        - Merges similar points bottom-up into a tree
    ...
    st.markdown("""
    1. Seasonal labels created from date (Winter/Spring/Summer/Autumn) for color coding only
    2. All numeric features selected, near-zero variance columns removed
    3. StandardScaler applied (mean=0, std=1)
    4. PCA reduced to 3D : retained 83.3% of variance
    """)
    st.divider()
...
with tabs[5]:
    ...
    st.markdown("""
    Association Rule Mining discovers relationships between variables in a dataset : specifically,
    which combinations of conditions tend to occur together. In agriculture, this can reveal which
    combinations of weather and soil conditions reliably co-occur with low soil moisture or high ET.
    A rule takes the form X → Y, meaning "when X is observed, Y also tends to occur."
    """)
    col1, col2, col3 = st.columns(3)
    with col1:
        st.info("**Support** = P(X ∩ Y)\nHow often the rule appears in the dataset.")
    with col2:
        st.info("**Confidence** = P(Y | X)\nHow reliable the rule is when X is present.")
    with col3:
        st.info("**Lift** = Confidence / P(Y)\nLift > 1 means a genuine association, not random.")
    st.markdown("""
    The **Apriori Algorithm** finds all frequent itemsets (support ≥ threshold) then generates rules
    filtered by minimum confidence. It prunes the search space by using the fact that any subset
    ...
    st.markdown("""
    ARM needs binary transaction data : each row is one day, each column is a binned condition.
    Continuous features were discretized into Low/Mid/High bins using quantile-based cutting:

    | Feature | Bins Created |
    |---------|-------------|
    ...
    `min_support = 0.05`, `min_confidence = 0.40`
    """)
    st.divider()
...
with tabs[6]:
    ...
    **Each NB flavor needs a different data format : here is what was applied:**
    ...
    st.markdown("**[Link to Dataset](https://github.com/shivanibhinge/AgriSense_Data_Driven_Irrigation_Insights/tree/main)** &nbsp;|&nbsp; **[Link to Colab Notebook](https://github.com/shivanibhinge/AgriSense_Data_Driven_Irrigation_Insights/tree/main)**")
    st.divider()
